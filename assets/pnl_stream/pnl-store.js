...
(function(global){
  ...
  class PnLStore {
    ...
    // External hooks
    setGetters({ getCash, getPositions, getMark, getRealized }){
    ...
    _computeEquity(){
      ...
      for (const sym in positions) {
        ...
        const qty = (typeof pos === "number") ? pos : (pos && pos.qty) ? pos.qty : 0;
        const lastKnown = (this.getMark ? this.getMark(sym) : undefined);
        const last = (lastKnown != null) ? lastKnown : (pos && pos.last != null ? pos.last : 0);
        equity += (qty || 0) * (last || 0);
      }
      return equity;
    }
    ...
    snapshot(){
      const out = {
        ...
        realized: (this.getRealized ? this.getRealized() : this.realized),
