...
(function(){
  ...
  function initOnce(){
    ...
    const getRealized = ()=>{
      const s = pickState();
      const v = (s.realized ?? s.pnlRealized ?? s.realizedPnl ?? s.realizedPNL);
      if (typeof v === 'number') return v;
      try{ if (window.__PnLLastRealized != null) return window.__PnLLastRealized; }catch(e){}
      try{
        const el = document.querySelector('[data-realized]');
        if (el){ const n = Number(el.getAttribute('data-realized') || el.textContent.replace(/[^\d.-]/g,'')); if (!Number.isNaN(n)) return n; }
      }catch(e){}
      return 0;
    };
const getCash = ()=>{
  ...
  return 0;
    ...
    try{
      ...
      window.PnLBridge.init({
        getCash, getPositions, getMark, getRealized,
        mountKpiEl: kpi,
