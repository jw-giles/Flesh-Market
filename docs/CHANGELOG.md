...
**Brighter allocation donuts.** Both the personal P&L net-worth donut (`_drawDonut`) and the Capital House NAV donut (`renderFundDonut`) drew flat opaque slices on the near-black panel and read as dim. Now each slice fills with a per-segment glow (`shadowColor`/`shadowBlur 6`) so the ring reads as saturated color glowing on black, the rings are slightly bolder (outer radius +2px, inner ratio 0.6), center text is warmer (`#ffcf7a` value, soft tan label), and the cash slice is lifted. Both palettes are now one shared vivid set (`FM_DONUT_PAL`, defined in `core.js`, read by `funds.js`), so the personal and Capital House wheels are identical. Luminosity comes from the glow, not from lightening the colors.
