...
// === BLACKJACK — Poker-style UI with AI players ===
(function(){
  'use strict';
  const pane=document.getElementById('casino-blackjack');
  if(!pane) return;

  // ── Shared card CSS (reuse poker style) ─────────────────────────
...
.bj-table{background:radial-gradient(ellipse at center,#0e3d1e 0%,#061a0b 100%);border:2px solid #2d5a1e;border-radius:16px;padding:16px 20px 20px;margin-bottom:14px}
.bj-section{margin-bottom:10px;min-height:36px}
.bj-label{font-size:.75rem;letter-spacing:.08em;color:#6a9a70;margin-bottom:4px}
.bj-cards{display:flex;gap:6px;flex-wrap:wrap;min-height:56px}
.bj-card{width:40px;height:54px;border-radius:5px;border:1px solid #555;background:#fff;display:flex;flex-direction:column;align-items:center;justify-content:center;font-size:.85rem;font-weight:bold;position:relative;line-height:1.1}
.bj-card.back{background:linear-gradient(135deg,#2a0a6a,#4a1a9a);border-color:#6a3aaa;color:#9a6aca;font-size:1.4rem}
.bj-card.red{color:#cc2200}
.bj-card.black{color:#111}
.bj-rank{font-size:.9rem;font-weight:900}
.bj-suit{font-size:.75rem}
.bj-info{display:flex;gap:20px;margin-bottom:10px;font-size:.85rem;flex-wrap:wrap}
.bj-info span{color:#8ab}
.bj-info strong{color:#e6c27a}
.bj-bet-row{display:flex;align-items:center;gap:8px;margin-bottom:8px;flex-wrap:wrap}
.bj-bet-row input{width:70px;padding:4px 6px;background:#0d0d08;border:1px solid #4a3a10;color:#e6c27a;font-size:.85rem;font-family:monospace;border-radius:4px}
.bj-chips{display:flex;gap:4px;flex-wrap:wrap}
.bj-chips button,.bj-actions button{padding:5px 10px;background:#1a1500;border:1px solid #5a4a10;color:#e6c27a;cursor:pointer;border-radius:4px;font-family:monospace;font-size:.8rem}
.bj-chips button:hover,.bj-actions button:hover{background:#2a2200}
.bj-actions{display:flex;gap:6px;margin-bottom:10px;flex-wrap:wrap}
...
.bj-result{padding:8px 12px;border-radius:6px;font-size:.95rem;margin:6px 0}
...
#bj-log{max-height:100px;overflow-y:auto;font-size:.75rem;color:#7a9a7a;line-height:1.5}
#bj-log div{border-bottom:1px solid #1a2a1a;padding:1px 0}
/* AI seats */
.bj-seats{display:flex;gap:8px;flex-wrap:wrap;margin-bottom:12px}
.bj-seat{background:#0a1a0a;border:1px solid #2a4a2a;border-radius:8px;padding:6px 10px;min-width:100px;flex:1}
.bj-seat.folded{opacity:.4}
.bj-seat.bust{border-color:#6a1a1a;background:#1a0808}
.bj-seat.stood{border-color:#4a4a00;background:#1a1a00}
.bj-seat.winner{border-color:#ffd700;background:#0a1a00}
.bj-seat.bj-hand{border-color:#60c8ff;background:#060f18}
.bj-seat .seat-name{font-size:.75rem;color:#8ab;letter-spacing:.06em;margin-bottom:2px}
.bj-seat .seat-stack{font-size:.8rem;color:#e6c27a}
.bj-seat .seat-bet{font-size:.7rem;color:#aaa;min-height:14px}
.bj-seat .seat-status{font-size:.7rem;color:#4ecdc4;min-height:14px;font-style:italic}
.bj-seat .seat-cards{display:flex;gap:3px;margin-top:4px;min-height:30px}
.bj-seat .seat-cards .bj-card{width:22px;height:30px;font-size:.55rem}
.bj-seat .seat-cards .bj-card.back{font-size:.9rem}
    `;
  ...
  pane.innerHTML=`
<div id="bj-wrap">
  <h3>Blackjack — 6-max vs AI</h3>
  <div class="bj-info">
    <span>Stack: <strong id="bj-balance">Ƒ0</strong></span>
    <span>Shoe: <strong id="bj-shoe-lbl">6 decks</strong></span>
  </div>
  <div class="bj-table">
    <div class="bj-label">AI PLAYERS</div>
    <div class="bj-seats" id="bj-seats"></div>
    <div class="bj-section">
      <div class="bj-label">DEALER <span id="bj-dealer-total" style="color:#e6c27a;font-size:.8rem"></span></div>
      <div class="bj-cards" id="bj-dealer-hand"></div>
    </div>
    <div class="bj-section">
      <div class="bj-label">YOUR HAND <span id="bj-player-total" style="color:#e6c27a;font-size:.8rem"></span></div>
  ...
  <div class="bj-actions" id="bj-actions">
    <button id="bj-btn-deal" onclick="bjDeal()">Deal Hand</button>
  ...
  <div class="bj-muted" style="font-size:.72rem;opacity:.5;margin-bottom:6px">6-deck shoe • Dealer stands soft 17 • Blackjack 3:2 • No splits/insurance</div>
  <div id="bj-result-box"></div>
  <div id="bj-log" style="margin-top:6px"></div>
</div>
`;
  ...
  function cardEl(c,faceDown=false){
    ...
    if(faceDown){ d.className='bj-card back'; d.innerHTML='🂠'; return d; }
  ...
  let aiPlayers=AI_NAMES.map(name=>({name,stack:500,hand:[],bet:0,status:'idle'}));

  function aiRebuy(ai){ if(ai.stack<20) ai.stack+=500; }

  // Basic strategy: stand on 17+, hit on soft 17 or below
  function aiDecide(hand,dealerUp){
    const tot=handTotal(hand);
    const soft=hand.some(c=>c.r==='A')&&tot<=17&&hand.length>=2;
    if(isBust(hand)) return 'bust';
    if(isBJ(hand)) return 'stand';
    if(tot>17) return 'stand';
    if(tot===17&&!soft) return 'stand';
    // Double on 10/11 vs dealer low card
    if(hand.length===2&&(tot===10||tot===11)&&dealerUp<=9) return 'double';
    return 'hit';
  }

  function renderSeats(showCards=false){
    const container=document.getElementById('bj-seats'); if(!container) return;
    container.innerHTML='';
    aiPlayers.forEach((ai,i)=>{
      const seat=document.createElement('div');
      let cls='bj-seat';
      if(ai.status==='bust') cls+=' bust';
      else if(ai.status==='stood'||ai.status==='doubled') cls+=' stood';
      else if(ai.status==='bj') cls+=' bj-hand';
      seat.className=cls;
      seat.id=`bj-seat-${i}`;
      const tot=ai.hand.length?handTotal(ai.hand):'';
      const statusTxt={idle:'',playing:'playing…',hit:'hit',stood:'stood',doubled:'doubled',bust:'BUST',bj:'BLACKJACK!',winner:'✓ WIN',loser:'✗ lose',push:'PUSH'}[ai.status]||ai.status;
      seat.innerHTML=`
        <div class="seat-name">${ai.name}</div>
        <div class="seat-stack">${fmtLocal(ai.stack)}${ai.bet?` · bet ${fmtLocal(ai.bet)}`:''}${tot?` · <b>${tot}</b>`:''}</div>
        <div class="seat-status">${statusTxt}</div>
        <div class="seat-cards" id="bj-ai-cards-${i}"></div>`;
      container.appendChild(seat);
      const cardRow=document.getElementById(`bj-ai-cards-${i}`);
      if(cardRow&&ai.hand.length){
        ai.hand.forEach((c,j)=>{
          cardRow.appendChild(cardEl(c,!showCards&&j===1&&ai.status==='idle'));
        });
      }
    });
  }
  ...
  function renderHands(hideHole=true){
    ...
    if(dTot) dTot.textContent=hideHole&&dealerHand.length>=2?`(${handTotal([dealerHand[0]])}+?)`:handTotal(dealerHand);
    if(pTot) pTot.textContent=handTotal(playerHand);
  }
  ...
  function settle(){
    renderHands(false); renderSeats(true);
    ...
    aiPlayers.forEach(ai=>{
      if(!ai.bet||ai.status==='bust'){ ai.status=ai.status==='bust'?'bust':'idle'; return; }
      const aTot=handTotal(ai.hand);
      if(dBJ&&!isBJ(ai.hand)){ ai.status='loser'; }
      else if(isBJ(ai.hand)&&!dBJ){ ai.stack+=Math.floor(ai.bet*1.5)+ai.bet; ai.status='winner'; }
      else if(isBust(dealerHand)||aTot>dTot){ ai.stack+=ai.bet*2; ai.status='winner'; }
      else if(aTot===dTot){ ai.stack+=ai.bet; ai.status='push'; }
      else { ai.status='loser'; }
      ai.bet=0;
    });
    renderSeats(true);
    ...
    clearTmr=setTimeout(resetRound,6000);
  }
  ...
  async function runAIs(){
    ...
    for(let i=0;i<aiPlayers.length;i++){
      ...
      if(!ai.hand.length||ai.status==='bust'||isBJ(ai.hand)) continue;
      ai.status='playing';
      renderSeats(false);
      await sleep(400);
      let action=aiDecide(ai.hand,handTotal([dealerUp]));
      while(action==='hit'||action==='double'){
        if(action==='double'){
          const extra=Math.min(ai.bet,ai.stack-ai.bet);
          ai.stack-=extra; ai.bet+=extra;
          ai.hand.push(shoe.draw()); ai.status='doubled';
          renderSeats(false); await sleep(350); break;
        } else {
          ai.hand.push(shoe.draw());
          if(isBust(ai.hand)){ ai.status='bust'; break; }
          renderSeats(false); await sleep(350);
          action=aiDecide(ai.hand,handTotal([dealerUp]));
        }
      }
      if(ai.status==='playing') ai.status='stood';
      renderSeats(false);
      await sleep(200);
    }
  }
  ...
  function resetRound(){
    ...
    aiPlayers.forEach(ai=>{ aiRebuy(ai); ai.hand=[]; ai.bet=0; ai.status='idle'; });
    renderHands(false); renderSeats(false);
  ...
  window.bjDeal=async function(){
    ...
    aiPlayers.forEach(ai=>{
      aiRebuy(ai);
      const ab=Math.min(Math.max(10,Math.floor(Math.random()*60)+10),ai.stack);
      ai.stack-=ab; ai.bet=ab; ai.hand=[]; ai.status='idle';
    });

    // Deal cards
    playerHand=[shoe.draw()]; dealerHand=[shoe.draw()];
    aiPlayers.forEach(ai=>ai.hand.push(shoe.draw()));
    playerHand.push(shoe.draw()); dealerHand.push(shoe.draw());
    aiPlayers.forEach(ai=>ai.hand.push(shoe.draw()));

    canDouble=true;
    setPhase('player'); setBtns('player');
    renderHands(true); renderSeats(false);
  ...
  window.bjHit=function(){
    ...
    if(isBust(playerHand)){
      ...
      setTimeout(async()=>{ await runAIs(); await runDealer(); },400);
    }
  };
