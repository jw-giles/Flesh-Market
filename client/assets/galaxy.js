...
function renderMarketsTab(){
  ...
  Promise.all([
    ...
    gToken ? fetch('/api/warehouses?token='+encodeURIComponent(gToken)).then(function(r){return r.json();}) : Promise.resolve({ok:true,sheds:[]})
  ]).then(function(res){
    var grid=res[0], cg=res[1], tr=res[2], sh=res[3], wh=res[4];
    window._gWh = wh || {sheds:[]};
    ...
    .then(function(r){return r.json();}).then(function(d){
      if(!d.ok){
        var msg=d.error==='no_ship'?'Commission a ship first (Shipyard below)':
          ...
          d.error==='shipment_in_progress'?'You already have a shipment in transit, only one at a time':(d.message||d.error||'Ship failed');
